...
class Asteroid {
  ...
  genShape() {
    ...
    for (let i = 0; i < TAU; i += TAU / 24) {
      let angleVector = p5.Vector.fromAngle(i).mult(random(this.size * (3.3 / 4), this.size));
      shape.push({
        x: angleVector.x,
        y: angleVector.y,
      });
  ...
  destroy(asteroids) {
    ...
    if (this.size >= 20) {
      for (let i = 0; i < 3; i++) {
        ...
        nAsteroid.position = createVector(
          random(this.position.x - this.size, this.position.x + this.size),
          random(this.position.y - this.size, this.position.y + this.size)
        );
        nAsteroid.clones = [];
        for (let i = 0; i < 8; i++) {
